...
train_metrics = pd.DataFrame(model.eval_metrics(train_data, ['RMSE'])['RMSE'])
val_metrics = pd.DataFrame(model.eval_metrics(val_data, ['RMSE'])['RMSE'])

train_metrics.columns = ['train_rmse']
val_metrics.columns = ['val_rmse']
